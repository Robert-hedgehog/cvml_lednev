...
class SimpleDetector(nn.Module):
    ...
    def forward(self, x):
        ...
        features_fc = self.fc_cls(features)
        features_bbox = self.fc_bbox(features)
        return self.cls_head(features_fc), torch.sigmoid(self.bbox_head(features_bbox))
...
def detection_loss(cls_pred, bbox_pred, cls_targets, bbox_targets, lambda_bbox=50.0):
...
best_acc = 0.0
no_improve = 0
best_val_loss = 10000000000000000.0
